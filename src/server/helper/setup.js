 ...
 */
function convertDateToDirectoryFormat(date) {
 ...
 */
function execSetup(system, success, error) {
    console.log(
    ...
    let todayDate = convertDateToDirectoryFormat(new Date());
    let lastUpdateDate = convertDateToDirectoryFormat(system.lastUpdateDate);
    ...
    fs.readdirSync(setupPath)
        ...
        .map(file => {
            let setupFunction = require(`./../setup/${file}`).default;
...
export default function (success, error) {
    ...
    SystemModel.findOne({})
        ...
        .then(system => {
            execSetup(system, success, error);
        });
}